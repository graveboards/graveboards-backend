...
SCHEMAS = {
    "beatmap": {
        "required_fields": ["beatmap_id", "difficulty_rating", "total_length"],
        "type_checks": {
            "beatmap_id": int,
    ...
    "beatmapset": {
        "required_fields": ["beatmapset_id", "title", "artist"],
        "type_checks": {
            "beatmapset_id": int,
            "title": str,
            "artist": str,
    ...
    "user": {
        "required_fields": ["user_id", "username", "country_code"],
        "type_checks": {
            "user_id": int,
            "username": str,
            "country_code": str,
    ...
    for field, expected_type in schema.get("type_checks", {}).items():
        if field in data:
            if not isinstance(data[field], expected_type):
                return False, f"Field '{field}' expected {expected_type}, got {type(data[field]).__name__}"
