...
def check_category_health(category: str, expected_count: Optional[int] = None) -> FixtureHealthResult:
    ...
    if not fixture_path.exists():
        expected = expected_count if expected_count is not None else calculate_fixture_counts().get(category, 0)
    ...
    coverage = (actual_count / expected_count * 100) if expected_count > 0 else (100.0 if actual_count == 0 else 0.0)
