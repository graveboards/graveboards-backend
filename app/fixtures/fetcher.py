...
class FixtureDataFetcher:
    ...
    Uses FetchLoop for the common fetch pattern (ID generation -> API call ->
    validation -> write -> metadata update). Category-specific wrapper logic
    handles multi-ruleset/multi-type iteration and special ID sources.
    """
    ...
    def _make_fetch_config(
        ...
        retries = (
            ...
            else (MAX_RETRIES_SCORES if self.force_fetch else MAX_RETRIES)
        )
        ...
        return FetchConfig(
            ...
            on_empty_data_limit=on_empty_data_limit,
        )
    ...
    async def fetch_beatmaps(
        ...
        loop = FetchLoop(config)
        fetched = 0
        async for event in loop.run(count, skip_existing=skip_existing):
            fetched = event.current
            yield event
    ...
    async def fetch_users(
        ...
        for ruleset in RULESETS:
            ...
            config = FetchConfig(
                ...
                success_handler=success_handler,
                failure_handler=failure_handler,
                skip_checker=self._should_skip_id,
                max_retries=MAX_RETRIES,
                max_attempts=count * 50 if self.force_fetch else count * 10,
                on_error=self._check_connection_stability,
            )
            loop = FetchLoop(config)
            async for event in loop.run(count, skip_existing=skip_existing):
                fetched[ruleset] = event.current
                cumulative_count += 1
    ...
    async def fetch_users_by_ids(
            ...
            while retries < MAX_RETRIES:
                try:
                    ...
                    if isinstance(e, httpx.HTTPStatusError) and e.response.status_code == 404:
                        ...
                        break
                    self.logger.debug(
                        f"Failed to fetch user {user_id} ({ruleset}) (retry {retries + 1}/{MAX_RETRIES}): {clean_error_msg(e)}"
                    )
                    await self._add_failed_id(f"users.{ruleset}", user_id)
                    retries += 1
        ...
        self.metadata["samples"]["users"]["per_ruleset"] = {
            r: self.metadata["samples"]["users"]["per_ruleset"].get(r, 0)
        ...
        self._current_session_results["users"] = {
            r: (fetched if r == ruleset else 0) for r in RULESETS
        }
        self.last_fetch_results = {
            "beatmaps": 0,
            "beatmapsets": 0,
    ...
    async def fetch_scores(
        ...
        for score_type in SCORE_TYPES:
            ...
            loop = FetchLoop(config)
            async for event in loop.run(count, skip_existing=skip_existing):
                fetched[score_type] = event.current
                cumulative_count += 1
                ...
                try:
                    ...
                    if not scores:
                        ...
                        if not self.force_fetch and consecutive_empty >= max_consecutive_empty:
                            self.logger.warning(
                                f"Too many consecutive beatmaps with no scores ({consecutive_empty}), stopping beatmap_scores fetch"
                            )
                            break
                        retries += 1
                        continue
                    ...
                    if isinstance(e, httpx.HTTPStatusError) and e.response.status_code == 404:
                        ...
                        break
                    self.logger.debug(
                        f"Failed to fetch beatmap scores {beatmap_id} (retry {retries + 1}/{inner_retries}): {clean_error_msg(e)}"
                    )
                    await self._add_failed_id("beatmaps", beatmap_id)
                    retries += 1
        ...
        for i in range(count):
            ...
            while retries < inner_retries:
                ...
                try:
                    ...
                    self._add_fetched_id("beatmaps", beatmap_id)
                    self.logger.debug(
                        f"Fetched beatmap attributes {beatmap_id} ({fetched}/{count})"
                    )
                    break
                except Exception as e:
                    ...
                    if not self.force_fetch and consecutive_errors >= max_consecutive_errors:
                        self.logger.warning(
                            f"Too many consecutive beatmap attribute errors ({consecutive_errors}), stopping beatmap_attributes fetch"
                        )
                        break
                    self._check_connection_stability(e)
                    if isinstance(e, httpx.HTTPStatusError) and e.response.status_code == 404:
                        self.logger.debug(
                            f"Beatmap {beatmap_id} not found for attributes, skipping"
                        )
                        await self._add_failed_id("beatmaps", beatmap_id)
                        break
                    self.logger.debug(
                        f"Failed to fetch beatmap attributes {beatmap_id} (retry {retries + 1}/{inner_retries}): {clean_error_msg(e)}"
                    )
                    await self._add_failed_id("beatmaps", beatmap_id)
                    retries += 1
    ...
    async def fetch_all(self, sample_counts: dict) -> AsyncIterator[FetchEvent]:
        ...
        if users_osu > 0 or users_taiko > 0 or users_fruits > 0 or users_mania > 0:
            async for event in self.fetch_users(
                users_osu, users_taiko, users_fruits, users_mania, skip_existing=True
            ):
                yield event

        if scores_best > 0 or scores_firsts > 0 or scores_recent > 0:
            async for event in self.fetch_scores(
                scores_best, scores_firsts, scores_recent, skip_existing=True
            ):
                yield event
        ...
        if beatmap_attributes_count > 0:
            async for event in self.fetch_beatmap_attributes(
                beatmap_attributes_count, skip_existing=True
            ):
                yield event
    ...
    def _add_fetched_id(self, category: str, id_: int) -> None:
        ...
        fetched_ids = self.metadata.setdefault(
            "fetched_ids",
            {
                "beatmaps": [],
                "beatmapsets": [],
