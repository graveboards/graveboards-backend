...
@pytest.mark.asyncio
async def test_cascade_delete_request_on_user():
    ...
    async with db.session() as session:
        async with session.begin():
            user = await db.add(User, session=session, id=1014)
            queue = await db.add(Queue, session=session, user_id=1014, name="Test Queue", description="Test")
            
            beatmapset_data = {
                "id": 10014,
                "user_id": 1014,
            }
            beatmapset = await db.add(Beatmapset, session=session, **beatmapset_data)
            ...
            beatmapset_snapshot = await db.add(BeatmapsetSnapshot, session=session, **snapshot_data)
            ...
            request = await db.add(Request, session=session, **request_data)
            
            await db.delete(User, session=session, id=1014)
    ...
    async with db.session() as session:
        async with session.begin():
            result = await session.execute(
                select(Request).where(Request.user_id == 1014)
            )
            fetched_request = result.scalars().first()
