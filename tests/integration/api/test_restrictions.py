"""
Integration tests for queue restriction enforcement.

Tests restriction checks during request submission and restriction management via queue PATCH.
"""
import os
import pytest
...
class TestRestrictionsOnRequestSubmission:
    """Integration tests for restriction enforcement on POST /api/v1/requests."""
    ...
    @pytest.mark.asyncio
    async def test_request_submits_when_no_restrictions(
        self, TestClientWithMocks, valid_request_body, security_disabled
    ):
        """Test request succeeds when queue has no restrictions."""
    ...
    async def test_request_blocked_by_rate_limit(
        ...
        from app.database.models import Queue
        from app.database.models.queue_restriction import QueueRestriction
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_request_passes_when_inactive_restriction(
        self, TestClientWithMocks, valid_request_body, security_disabled
    ):
        """Test request succeeds when restriction exists but is inactive."""
        from app.database.models import Queue
    ...
    async def test_request_passes_when_user_not_in_target(
        ...
        """Test request succeeds when user is not in restriction target list."""
        from app.database.models import Queue
...
class TestQueueRestrictionsPatch:
    """Integration tests for PATCH /api/v1/queues/{id} with restrictions."""

    TEST_QUEUE_ID = 1

    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_admin_can_set_restrictions(self, TestClientWithMocks, admin_user_token):
        """Test admin can set restrictions via PATCH."""
        from app.database.models import Queue
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_queue_owner_can_set_restrictions(
        self, TestClientWithMocks, admin_user_token
    ):
        """Test queue owner can set restrictions via PATCH."""
        from app.database.models import Queue
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_non_admin_cannot_set_restrictions(
        self, TestClientWithMocks, admin_user_token
    ):
        """Test non-admin non-owner gets 403 when trying to set restrictions."""
