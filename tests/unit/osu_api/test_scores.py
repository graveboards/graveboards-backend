...
from tests._helpers.osu_api_helpers import _get_beatmap_scores_with_fallback
...
@pytest.mark.asyncio
async def test_get_beatmap_scores_with_offset(api_client: tuple[OsuAPIClient, MagicMock]) -> None:
    api_client_obj, mock_redis = api_client
    fixture_manager = FixtureReader()
    mock_data = _get_beatmap_scores_with_fallback(fixture_manager)
