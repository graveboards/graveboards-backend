...
class TestSafeCompileRegex:
    async def test_valid_regex_returns_compiled_pattern(self) -> None:
        pattern = r"test"
    ...
    async def test_valid_regex_with_special_characters(self) -> None:
        pattern = r"^[a-zA-Z0-9_]+$"
        result = await safe_compile_regex(pattern)

        assert result is not None

    async def test_invalid_regex_returns_none(self) -> None:
        pattern = r"[invalid"
        result = await safe_compile_regex(pattern)
        assert result is None

    async def test_safe_regex_matches_expected_text(self) -> None:
