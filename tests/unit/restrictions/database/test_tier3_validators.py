...
from app.database.restrictions.validators.database.never_ranked import (
    NeverRankedRestriction,
    NeverRankedConfig,
)
from app.database.restrictions.validators.database.unique_artist_title import (
...
class TestNeverRankedRestriction:
    ...
    async def test_passes_when_no_ranked_matches(self):
        restriction = NeverRankedRestriction()
    ...
    async def test_raises_when_ranked_match_found(self):
        ...
        with pytest.raises(RestrictionViolationError, match="already ranked"):
            with patch.object(ExecutionContext, "get_metadata", new=mock_provider.resolve):
                await restriction.check(context)
    ...
    async def test_raises_on_missing_identity(self):
        ...
        with pytest.raises(RestrictionViolationError, match="Could not resolve"):
            with patch.object(ExecutionContext, "get_metadata", new=mock_provider.resolve):
                await restriction.check(context)
...
class TestUniqueArtistTitleRestriction:
    ...
    async def test_passes_when_no_duplicate(self):
        restriction = UniqueArtistTitleRestriction()
        osu_client = _make_mock_osu_client({
            "beatmapsets": [
    ...
    async def test_raises_when_duplicate_found(self):
        ...
        with pytest.raises(RestrictionViolationError, match="already has a ranked request"):
            with patch.object(ExecutionContext, "get_metadata", new=mock_provider.resolve):
                await restriction.check(context)
