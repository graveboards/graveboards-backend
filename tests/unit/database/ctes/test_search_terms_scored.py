...
class TestProcessFieldGroups:
    ...
    def test_process_field_groups_with_groups(self):
        ...
        base_query = union_all(
            select(
                literal(1).label("id"),
                literal("title").label("field"),
                literal("term").label("term"),
                literal("pattern").label("pattern"),
                literal(10).label("score"),
            ),
            select(
                literal(2).label("id"),
                literal("title_unicode").label("field"),
                literal("term").label("term"),
                literal("pattern").label("pattern"),
                literal(15).label("score"),
            ),
        ).subquery()

        field_groups_config = {
            SearchableFieldCategory.BEATMAPSET: {"title": {"title", "title_unicode"}}
        }

        result = _process_field_groups(
            base_query, SearchableFieldCategory.BEATMAPSET, field_groups_config
        )
